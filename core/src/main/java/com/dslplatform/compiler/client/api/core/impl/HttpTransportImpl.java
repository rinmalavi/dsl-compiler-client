...
*/
    ...
    public HttpResponse sendRequest(final HttpRequest request) throws IOException {
        ...
        if (logger.isDebugEnabled()) {
            ...
            final StringBuilder sb = new StringBuilder();
            sb.append("Receiveddd ").append(body.length).append(" bytes in ").append(tookMs).append(" ms (from ")
            ...
            logger.debug(sb.toString());
            logger.debug("Response bodyyyyy");
            try { // todo - in trace usaly
                logger.debug("Response body: >{}<", new String(response.body, ENCODING));
            } catch (final Exception e) {
                logger.debug(">could not decode response body via UTF-8<");
            }
        }
